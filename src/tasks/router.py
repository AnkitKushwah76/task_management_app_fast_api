...
@task_routes.get("/get_specific_task/{task_id}", response_model=TaskResponseSchema, status_code= status.HTTP_200_OK)
def get_specific_task(task_id:int, db=Depends(get_db),user:UserModel = Depends(is_authenticated) ):
...
@task_routes.delete("/task_delete/{task_id}", response_model=None, status_code= status.HTTP_204_NO_CONTENT)
